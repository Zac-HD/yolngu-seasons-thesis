...
def save_figure(fig, station_name, name):
    ...
    fig.savefig("../output/{}/{}.pdf".format(station_name, name), bbox_inches="tight")
...
def season_prob(data):
    ...
    grp = data[is_seasons].groupby(data.index.dayofyear).sum()
...
def save_out(station):
    ...
    save_table(
        monthly,
        [nameof._asdict()[n] for n in utils.chart_panels],
        station_name,
        "monthly-summary",
    )
    save_table(
        monthly,
        ["Detected Season"] + list(utils.seasons),
        station_name,
        "monthly-seasons",
    )
    ...
    save_figure(multipanel(data, *utils.chart_panels), station_name, "observations")
    save_figure(
        multipanel(data, *[w for w in utils.nameof._fields if "windspd" in w]),
        station_name,
        "seabreeze-speed",
    )
    save_figure(
        multipanel(data, *[w for w in utils.nameof._fields if "winddir" in w]),
        station_name,
        "seabreeze-direction",
    )
    save_figure(
        multipanel(data, "Detected Season", *utils.seasons), station_name, "seasons"
    )
    save_figure(lines(daily[list(utils.seasons)]), station_name, "seasons-daily-index")
    save_figure(
        lines(
            season_prob(data),
            filled=True,
    ...
    save_figure(
        timing_circle(daily[list(utils.seasons)], season_prob(data)),
        station_name,
        "timing-circle",
    )
...
def season_pie(data):
    ...
    vcs.name = ""
    vcs.plot.pie(ax=ax, legend=False, figsize=(3, 3), startangle=90, counterclock=False)
    return fig
...
def timing_circle(daily_seasons, obvs):
    ...
    def occur_interval(series, interval=0.9):
        ...
        for i in range(365):
            ...
            cs = (s / s.sum()).cumsum()
            candidates.append(
                (cs[cs <= delta].index[-1], cs[cs >= (interval + delta)].index[0])
            )
    ...
    fig, ax = plt.subplots(figsize=(8, 8), dpi=600, subplot_kw={"projection": "polar"})
    ...
    for i, s in enumerate(utils.seasons):
        ...
        for it in (0.5, 0.75, 0.9):
            ...
            ax.plot(
                radians, dat * i, linewidth=(1 - it) * 25, color=sns.color_palette()[i]
            )
    return fig
...
def climograph(df):
    ...
    with sns.axes_style("dark"):
        ...
        # Plot total rainfall
        ax_rain.bar(
            m.mean().index, m[utils.nameof.rain].mean(), width=0.6, align="center"
        )
...
def wind_vectors_plot(data):
    def count_by_month(series):
        return [
            g.value_counts(dropna=True, sort=False)
            for name, g in (
                series.replace("CALM", np.NaN)
    ...
    for ax, name, title in (
        (axes[0], nameof.winddir09, "Morning (9am)"),
...
def heatmap(data, kind, **kwargs):
    ...
    base_kwargs = {
        ...
        "yticklabels": ["" if d % (len(data.index) // 5) else d for d in data.index],
        "robust": True,
    ...
    temp = {"cmap": "coolwarm"}
    wspd = {"vmin": 10, "vmax": 40, "cmap": sns.light_palette("navy", as_cmap=True)}
    wdir = {
    ...
    kind_kwargs = {
        ...
        **{
            "windspd"
            + hr: {"cbar_kws": {"label": text + " wind\nspeed (km/h)"}, **wspd}
            for hr, text in zip(utils._wind_hours, utils._hour_names)
        },
        **{
            "winddir" + hr: {"cbar_kws": {"label": text + " wind\ndirection"}, **wdir}
            for hr, text in zip(utils._wind_hours, utils._hour_names)
        },
        "Detected Season": {
            "robust": False,
    ...
    hax.set_title(
        all_kwargs["cbar_kws"]["label"].replace("\n", " ").split("(")[0],
        loc="left",
        y=0.96,
        fontsize=10,
    )
...
def multipanel(df, *cols, **kwargs):
    ...
    context = {"axes.facecolor": "black", "figure.figsize": (10, 1.5 * len(cols))}

    func = {  # any needed data transformations
        "rain": lambda df, name: utils.pivot(
            df[nameof.rain]
            .rolling(center=True, window=5, min_periods=1)
            .mean()
            .to_frame(),
            name,
        ),
        **{"winddir" + h: utils.categorical_to_numeric_wind for h in utils._wind_hours},
