...
def quantiles(series, low, high):
    ...
    return ((series >= series.quantile(low)) & (series <= series.quantile(high))).where(
        series.notnull()
    )
...
def wind_between(df, start, end, *, am=True, pm=True):
    ...
    am_wind = sum([df[am_name] == d for d in directions]).where(df[am_name].notnull())
    pm_wind = sum([df[pm_name] == d for d in directions]).where(df[pm_name].notnull())
...
def season_indicies(data):
    ...
    weekly_rain_days = (
        (data[nameof.rain] > 1).rolling(window=7, center=True, min_periods=5).sum()
    )
    ...
    season["Midawarr"] = sum(
        [wind_between(data, "NE", "ESE"), (weekly_rain_days != 0) * 0.5]
    )
    season["Dharrathamirri"] = sum(
        [weekly_rain_days == 0, wind_between(data, "ESE", "SSE")]
    )
